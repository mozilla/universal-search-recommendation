...
```sh
git clone https://github.com/mozilla/ghosttown.git
cd ghosttown
mkvirtualenv ghosttown  # You may need to set --python to point to the python 3.5 executable.
...
- `GHOSTTOWN_ENV`: the environment in which the server is being run. Options: `development`, `staging`, `testing`, `production`. Default: `development`.
- `GHOSTTOWN_HOST`: the host at which you'd like the server to run. Default: `0.0.0.0`
- `GHOSTTOWN_PORT`: the port at which you'd like the server to listen. Default: `5000`
